...
#define PLUGIN_NAME "Vision"

class Vision : public IPlugin
{
public:
    void Main();
    void run();
...
void Vision::Main()
{
    ...
    GUI::GetInstance();
    IplImage *frame;
    current_tab = new tab("Vision");
    vid = new video(0,HEIGHT*.02,WIDTH/2,HEIGHT/2,"Vision", current_tab);
    initFaceDet("haarcascade_frontalface_default.xml");
    while (true)
    {
        frame=cvCloneImage( patrol->get_Instance().get_image());
        detectFace(frame,&polo);
        vid->SetImage(frame);
