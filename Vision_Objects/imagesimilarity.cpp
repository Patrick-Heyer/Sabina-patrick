       ...
       for(int i=0;i<h;i++)
           for(int j=0;j<w;j++){
               ...
               uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
                   ...
                   double h1=2*double(ptr1[3*j]);       double h2=2*double(ptr2[3*j]);
                   double s1=double(ptr1[3*j+1])/255.0;  double s2=double(ptr2[3*j+1])/255.0;
                   double v1=double(ptr1[3*j+2])/255.0;  double v2=double(ptr2[3*j+2])/255.0;
...
for(int i=0;i<h;i++)
    for(int j=0;j<w;j++){
        ...
        uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
            ...
            double h1=double(ptr1[3*j])/255;   double h2=double(ptr2[3*j])/255;
            double s1=double(ptr1[3*j+1])/255;  double s2=double(ptr2[3*j+1])/255;
            double v1=double(ptr1[3*j+2])/255;  double v2=double(ptr2[3*j+2])/255;
...
for( int i=0; i<N; ++i )
{
   ...
   for( int j=0; j<M; ++j )
   {
       ...
       uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
                   ...
                   double r1=double(ptr1[3*j]);   double r2=double(ptr2[3*j]);
                   double g1=double(ptr1[3*j+1]);  double g2=double(ptr2[3*j+1]);
                   double b1=double(ptr1[3*j+2]);  double b2=double(ptr2[3*j+2]);
...
for( int i=0; i<N; ++i )
{
   ...
   for( int j=0; j<M; ++j )
   {
       ...
       uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
               ...
               if(spaceColorC1c2c3){
               r1=double(ptr1[3*j])/255;    r2=double(ptr2[3*j])/255;
               g1=double(ptr1[3*j+1])/255;   g2=double(ptr2[3*j+1])/255;
               b1=double(ptr1[3*j+2])/255;   b2=double(ptr2[3*j+2])/255;

                }
               else{
                r1=double(2*ptr1[3*j])/360;    r2=double(2*ptr2[3*j])/360;
                g1=double(ptr1[3*j+1])/255;   g2=double(ptr2[3*j+1])/255;
                b1=double(ptr1[3*j+2])/255;  b2=double(ptr2[3*j+2])/255;
   ...
   for( int j=0; j<M; ++j )
   {
    ...
    contador1++;
       ...
       if(spaceColorC1c2c3){
                   ...
                   { r1=double(ptr1[3*j])/180;   r2=double(ptr2[3*j])/180;}

                   double g1=double(ptr1[3*j+1])/255;  double g2=double(ptr2[3*j+1])/255;
                   double b1=double(ptr1[3*j+2])/255;  double b2=double(ptr2[3*j+2])/255;
...
for( int i=0; i<N; ++i )
{
   ...
   for( int j=0; j<M; ++j )
   {
       ...
       uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
               ...
               double r1=0;
               double r2=0;
               r1=double(ptr1[3*j])/double(maxvalue);    r2=double(ptr2[3*j])/double(maxvalue);
   ...
   for( int j=0; j<M; ++j )
   {
    ...
    contador1++;
       ...
       uchar *ptr2 = (uchar*) (img2->imageData + i*img2->widthStep);
                   ...
                   double r1=double(ptr1[3*j])/double(maxvalue);   double r2=double(ptr2[3*j])/double(maxvalue);
