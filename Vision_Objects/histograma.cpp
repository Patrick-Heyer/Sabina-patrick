  ...
  void Histograma::CalcularHistogramaConjunto3Canal(IplImage *img1, IplImage *img2, cv::Mat &hist, int nh, int ns, int nv){
      ...
      //checar que las imagenes sean del mismo tamaño
          ...
          for(int j=0; j<w;j++){
            ...
            //     uchar *ptr1=ptr2;
                     int h1=(ptr1[3*j])/(256/double(nh));   int h2=(ptr2[3*j])/(256/double(nh));
                     int s1=(ptr1[3*j+1])/(256/double(ns));  int s2=(ptr2[3*j+1])/(256/double(ns));
                     int v1=(ptr1[3*j+2])/(256/double(nv));  int v2=(ptr2[3*j+2])/(256/double(nv));
...
void Histograma::CalcularHistogramaConjuntoHSV(IplImage *img1, IplImage *img2, cv::Mat &hist, int nh, int ns, int nv){
    ...
    //checar que las imagenes sean del mismo tamaño
        ...
        for(int j=0; j<w;j++){
          ...
          //     uchar *ptr1=ptr2;
                   int h1=(ptr1[3*j])/(180/double(nh));   int h2=(ptr2[3*j])/(180/double(nh));
                   int s1=(ptr1[3*j+1])/(256/double(ns));  int s2=(ptr2[3*j+1])/(256/double(ns));
                   int v1=(ptr1[3*j+2])/(256/double(nv));  int v2=(ptr2[3*j+2])/(256/double(nv));
