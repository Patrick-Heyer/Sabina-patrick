												...
												void*				pCookie)
{
  ...
  //x es la z que da el kinect
  xhum=pPosition->Z;
  xhum=xhum-1000.00;
  //y es la y que da el kinect
  yhum=0;
  
  

  xbot=patrulla->getInstance().Position.get_X()*10;
  ybot=patrulla->getInstance().Position.get_Y()*-10;
  angbot=patrulla->getInstance().Position.get_Angle();
  angbot=angbot*(M_PI/180);
  
  
  x0=((xhum*cos(angbot))+(yhum*sin(angbot))+xbot);
  y0=((xhum*sin(angbot))+(yhum*cos(angbot))+ybot);
  
  Location temp;
	temp.set_X(x0);
	temp.set_Y(y0);
	temp.set_Angle(angbot);

if(gesto==0)
{
	patrulla->getInstance().Clear_path();
	patrulla->getInstance().Add_Location_to_path(temp);
	printf("move robot to(%f,%f,%f)\n", x0, y0, angbot);
	gesto++;
}
...
float posicionX (const XnPoint3D* pPosition )
{
 return pPosition->X;
}

float posicionY (const XnPoint3D* pPosition )
{
 return pPosition->Y;
}

float posicionZ (const XnPoint3D* pPosition )
{
 return pPosition->Z;
}
