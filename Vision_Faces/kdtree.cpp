                    ...
                    struct feature*** nbrs, int max_nn_chks )
{
    ...
    while( min_pq->n > 0  &&  t < max_nn_chks )
    {
        ...
        for( i = 0; i < expl->n; i++ )
        {
            ...
            bbf_data1 = (bbf_data*) malloc( sizeof( struct bbf_data ) );
            if( ! bbf_data1 )
            {
            ...
            bbf_data1->old_data = tree_feat->feature_data;
            bbf_data1->d = descr_dist_sq(feat, tree_feat);
            tree_feat->feature_data = bbf_data1;
    ...
    for( i = 0; i < n; i++ )
    {
        bbf_data1 = (bbf_data*) _nbrs[i]->feature_data;
        _nbrs[i]->feature_data = bbf_data1->old_data;
        free( bbf_data1 );
    }
    *nbrs = _nbrs;
    return n;
...
{
    struct kd_node* kd_node1;

    kd_node1 = (kd_node*)malloc( sizeof( struct kd_node ) );
    memset( kd_node1, 0, sizeof( struct kd_node ) );
    kd_node1->ki = -1;
    kd_node1->features = features;
    kd_node1->n = n;
