...
int CProbot::BuildRoadMap()
{
    ...
    else if (Parametros_m.SamplingMethod == PRM::GAUSSIAN_S )
    {
        ...
        cout<<"PRM: M�todo de muestreo elegido no existe"<<endl;
...
int CProbot::ProcesarChelas( int row1, int col1, double ori1,int row2, int col2, double ori2)
{
    ...
    for (i = 0; i < 10; i++)        //m�ximo 10 intentos, incremeta 10 nodos nuevos por intentos
    {
        BuildRoadMap();
...
int CProbot::ProcesarLostF( int row, int col, double ori)
{
    ...
    Limpiar();
    for (i = 0; i < 10; i++)        //m�ximo 10 intentos para generar las metas, se incrementa
    {   // 10 nodos al grafo
        BuildRoadMap();
        ...
        {
            cout<<"PRM: Estrateg�a de b�squeda terminada, con un total de metas: "<<listametas.size()<<endl;
            break;
        }
