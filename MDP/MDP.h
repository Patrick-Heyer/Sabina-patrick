...
/*! \fn std::string cambiar_estado(string variable, string valor);
 * \brief Changes a variable in the MDP
 * 
 * \param variable name of the variable to change
 * \param valor value the variable should have
 * \returns std::string optimal action selected by the MDP
 */
std::string cambiar_estado(string variable, string valor);

/*! \fn void Init_MDP( char *Filename);
 * \brief Changes a variable in the MDP
 * 
 * \param variable name of a policy file generated by SPUDD (by Dr. Jesse Hoey http://www.cs.uwaterloo.ca/~jhoey/research/spudd/index.php)
 */
void Init_MDP( char *Filename);
string valor_variable(string variable);
...
class MDP {
 ...
 public:
  ...
  DdNode *getQfunction(int a);  // Anexadaaa  sugerencia Jesse Hoey
