	...
	if (temp.find("GO")!= temp.npos)
	{
	  patrol->getInstance().set_Current_destination("EXIT");
	   ...
	   {	
	       ...
	       {
		   patrol->getInstance().Sintetizer.set_Phrase("i cant hear you plase click on your drink on the screen");
		   sleep(2);
		   for(int i=0; i<objects.size(); i++)
		   {
			   ...
			   {
			       destino_conocido=true;
			       *patrol->getInstance().Last_objective= obje_butt[i]->GetName().c_str();
	   ...
	   patrol->getInstance().Sintetizer.set_Phrase("Ok i am going to the kitchen for it ");
	   sleep(2);
	   
	   patrol->getInstance().set_Current_destination("KITCHEN");
	   cambiar_estado("ruta_planeada", "no");
	   cambiar_estado("destino_alcanzado","no");
	   patrol->getInstance().set_Action(cambiar_estado("objeto_solicitado","si"));
