...
void Voice_Recognition::Main()
{
    ...
    for (;;)
    {
        ...
        if(accion=="pedir_objeto")
        {
            //GUI::getInstance().Set_Active_Tab("Voice_Recognition");
            bool objeto_conocido=false;
            int tries=0;
            ...
            std::stringstream ss;
            ss << "Ok i am going to get your " << patrol->getInstance().Microphone.get_Phrase();
            patrol->getInstance().Sintetizer.set_Phrase(ss.str());
            sleep(1);
